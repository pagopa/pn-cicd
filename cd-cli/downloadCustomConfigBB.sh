...
usage() {
    ...
    [-c <PnConfigurationTag>]      : a partire dal file repository-list.json nel repo pn-bb-configuration 
...
parse_params() {
  ...
  aws_region=""
  env_type=""
  configuration_repository_secret_name="pn-bb-configurations-repository"
...
if ( [ ! -z "${configuration_repository_secret_name}" ] ) then 
  ...
  if ( [ "$secretPresent" -eq "1" ]) then
    ...
    aws ${aws_command_base_args} secretsmanager get-secret-value \
        --secret-id pn-bb-configurations-repository \
...
_clone_repository(){
  ...
  _DEPLOYKEY="deploykey/pn-bb-configuration"
  ...
  _GITURI="https://github.com/pagopa/pn-bb-configuration.git"
  ...
  if ( [ -z "${_AWSDEPLOYKEYEXIST}" ] ); then    
    ...
    _GITURI="git@github.com:pagopa/pn-bb-configuration.git"
  fi

  git clone ${_GITURI}

  (cd pn-bb-configuration && git reflog -n 10)
}
...
if ( [ ! -z "${PN_CONFIGURATION_TAG}" -a ! -z "${cicd_account_id}" ] ) ; then
  ...
  for PN_CONFIGURATION_TAG_param in $( cat pn-bb-configuration/$env_type/repository-list.json |  jq 'keys_unsorted'  | grep -E "Id|Url" | sed -E 's/"//g' | sed -E 's/,//g' | sed -E 's/ //g' ); do
    #Take for all components tag, branch, ImageUrl ecc...
    PN_COMMIT=$(echo "$( cat pn-bb-configuration/$env_type/repository-list.json | jq -r '.'\"$PN_CONFIGURATION_TAG_param\"'' )"); 
...
echo "****   EXPORT COMPLETED   ****"
  ...
  cp -r pn-bb-configuration/${env_type}/* custom-config/
